...
class CUAR_PrivateFilesDefaultHandlers
{
    ...
    public function __construct($plugin)
    {
        ...
        add_filter('cuar/private-content/files/on-remove-attached-file?source=local', array(&$this, 'remove_attached_local_file'), 10, 3);
        ...
        add_filter('cuar/private-content/files/on-remove-attached-file?source=server', array(&$this, 'remove_attached_server_file'), 10, 3);
    ...
    {
        $filename = isset($file['file']) ? $file['file'] : '';
        $is_protected = isset($file['is_protected']) ? $file['is_protected'] : 1;
        if ($is_protected)
        {
            /** @var CUAR_PostOwnerAddOn $po_addon */
            $po_addon = $this->plugin->get_addon('post-owner');
