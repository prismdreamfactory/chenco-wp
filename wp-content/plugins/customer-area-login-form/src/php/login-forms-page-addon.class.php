...
if ( !class_exists('CUAR_LoginFormsPageAddOn')) :
    ...
    abstract class CUAR_LoginFormsPageAddOn extends CUAR_AbstractPageAddOn
    {
        ...
        public function __construct($form_id, $hide_if_logged_in = true, $hide_in_menu = true)
        {
            parent::__construct('customer-' . $form_id);
        ...
        {
            printf('<form name="%1$s" method="post" class="cuar-%1$s-form" action="%2$s">', $this->form_id, $this->get_page_url());
        ...
        public function get_form_links()
        {
            ...
            {
                $form_links['login'] = array(
                    'url'   => $lf_addon->get_login_url(),
                    'label' => __('Login', 'cuarlf')
                );
            }

            if ($this->form_id != 'forgot-password')
            {
                $form_links['forgot-password'] = array(
                    'url'   => $lf_addon->get_forgot_password_url(),
                    'label' => __('Lost password?', 'cuarlf')
                );
            }

            if ($this->form_id != 'register' && get_option('users_can_register'))
            {
                $form_links['register'] = array(
                    'url'   => $lf_addon->get_register_url(),
                    'label' => __('Register', 'cuarlf')
                );
            }
