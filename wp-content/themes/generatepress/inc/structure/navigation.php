...
<nav id="site-navigation" <?php generate_do_element_classes('navigation'); ?>
  <?php generate_do_microdata('navigation'); ?>>
  <div <?php generate_do_element_classes('inside_navigation'); ?>>
    ...
    <button class="menu-toggle" aria-controls="primary-menu" aria-expanded="false">
      <?php
							...
							?>
      <span class="mobile-menu"><?php echo apply_filters('generate_mobile_menu_label', __('Menu', 'generatepress')); // WPCS: XSS ok. 
																				?></span>
    </button>
    <?php
						wp_nav_menu(
	...
	if (!class_exists('Generate_Page_Walker') && class_exists('Walker_Page')) {
		...
		class Generate_Page_Walker extends Walker_Page
		{
			function start_el(&$output, $page, $depth = 0, $args = array(), $current_page = 0)
			{
				...
				$output .= sprintf(
					'<li class="%s test"><a href="%s">%s%s%s%s</a>',
					$css_classes,
...
if (nav) {
  ...
  clone.setAttribute('aria-label', '<?php esc_attr_e('
    Mobile Menu ', '
    generatepress '); ?>');
